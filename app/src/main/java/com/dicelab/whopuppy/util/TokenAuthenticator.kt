...
class TokenAuthenticator(
    ...
    private fun getRefreshedJwtTokenResult(): Result<String> {
        ...
        retrofit.create(AuthApi::class.java).refreshToken()
            .retry(5L)
            .subscribe(
                { token ->
                    ...
                    Logger.d("new access token : ${token.accessToken}")
                    Logger.d("new refresh token : ${token.refreshToken}")
                ...
                { error ->
                    Logger.e("token update error : $error")
